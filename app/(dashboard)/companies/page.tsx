    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-800 rounded-xl pl-10 pr-4 py-2.5 text-xs text-neutral-900 dark:text-white placeholder:text-neutral-400 dark:placeholder:text-neutral-500 focus:outline-none focus:border-neutral-900 dark:focus:border-brand-aloe/50 shadow-sm"
          />
        </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs whitespace-nowrap transition-all ${
                selectedIndustry === ind
                  ? "bg-brand-aloe/30 dark:bg-brand-aloe/20 text-emerald-950 dark:text-brand-aloe border border-brand-aloe/60 dark:border-brand-aloe/40 font-semibold shadow-sm"
                  : "bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-800 text-neutral-700 dark:text-neutral-400 hover:text-neutral-950 dark:hover:text-white hover:bg-neutral-50"
              }`}
            >
              {ind}
            </button>
      ...
      {filtered.length === 0 && !isLoading && (
        <Card variant="glass" className="p-12 text-center space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center mx-auto text-neutral-500 dark:text-neutral-400">
            <Search className="w-6 h-6" />
          </div>
          <div className="space-y-1">
            <h3 className="text-sm font-medium text-neutral-950 dark:text-white">No companies found</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((c) => (
          <Card key={c.id || c.company_name} variant="glass" className="p-5 space-y-3 flex flex-col justify-between">
            <div className="space-y-2.5">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h3 className="font-semibold text-neutral-950 dark:text-white text-base tracking-tight">{c.company_name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    {c.website && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-[11px] text-emerald-700 dark:text-brand-mint hover:underline inline-flex items-center gap-1 font-mono"
                      >
                        {c.website.replace(/^https?:\/\//, "").replace(/\/$/, "")} <ExternalLink className="w-2.5 h-2.5" />
                      </a>
                    )}
                    {c.location && (
                      <span className="text-[10px] text-neutral-500 dark:text-neutral-400 inline-flex items-center gap-0.5 font-medium">
            ...
            <div className="pt-3 border-t border-neutral-200/70 dark:border-white/5 flex items-center justify-between text-xs">
              <span className="text-neutral-600 dark:text-neutral-400 text-[11px] inline-flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 dark:text-brand-aloe" /> {c.sponsor_rating || "Worker (A rating)"}
              </span>
              <Link href="/dashboard/emails">
